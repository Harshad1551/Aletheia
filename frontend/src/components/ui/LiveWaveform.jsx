...
const LiveWaveform = ({
  ...
  useEffect(() => {
    ...
    const animate = (currentTime) => {
      ...
      if (active && currentTime - lastUpdateRef.current > updateRate) {
        ...
        if (analyserRef.current) {
          ...
          if (mode === "static") {
            ...
            const currentBarCount = Math.floor(rect.width / (barWidth + barGap));
            const halfCount = Math.floor(currentBarCount / 2);
            const newBars = [];
      ...
      const currentBarCount = Math.floor(rect.width / step);
      ...
      if (mode === "static") {
        ...
        for (let i = 0; i < currentBarCount && i < dataToRender.length; i++) {
        ...
        for (let i = 0; i < currentBarCount && i < historyRef.current.length; i++) {
  ...
  return (
    ...
    >
      {!active && !processing && (
        <div className="absolute top-1/2 right-0 left-0 -translate-y-1/2 border-t-2 border-dotted" style={{ borderColor: 'rgba(107,107,107,0.2)' }} />
      )}
      <canvas
